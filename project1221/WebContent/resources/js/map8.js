	...
	for (var i = 0; i < keys.length; i++) {
		...
		for (let k = 0; k < linePath.length; k++) {
			...
			if (distance < distanceBar) {
				...
				routeForDistance.push([keys[i], distance / 1000]);
				break;
			}
		}
		...
		if (isInRadiusForAnyPoint) {
			...
			routeList.push([keys[i], i]);

			polylines.push({ index: i, polyline: polyline });

			polyline.setMap(null); // 일단 모두 숨김
		}
	}
}
function getRouteListSize() {
	// 코스리스트 넣을 곳
	...
	ulElement.classList.add("your-ul-class");  // ul에 클래스 추가



	// routelist는 상단 fun에 설정해 놓음 
	for (let i = 0; i < routeList.length; i++) {
		// li 태그 생성
		var liElement = document.createElement("li");
		liElement.id = "list_" + i;
		liElement.classList.add("your-li-class");  // li에 클래스 추가

		// span1Element 추가
		var span1Element = document.createElement("span");
		span1Element.classList.add("td1");

		// span3Element 추가
		var span3Element = document.createElement("span");
		span3Element.classList.add("td3");

		// spanContent 추가
		var spanContent = document.createElement("span");
		spanContent.classList.add("poi_content");  // 추가한 부분
		span3Element.appendChild(spanContent);  // span3Element에 추가

		// a 태그의 속성 설정
		var aElement = document.createElement("a");
		aElement.href = "#";
		aElement.classList.add("your-a-class");  // a에 클래스 추가

		// 클릭 이벤트 리스너 추가
		aElement.onclick = function(number, fileName) {
			return function() {
				// 클릭된 a 태그의 텍스트를 이용하여 지도의 중심을 변경
				console.log(number);
				courseview(number);
				handleATagClick(number);
				setview(number);
			};
		}(routeList[i][1], routeList[i][0]);

		let courseArray = items[routeList[i][1]];
		// 해당하는 인덱스에 있는 코스 파일 가져오기
		...
		// number 가 routeList[i][1], fileName 은 routeList[i][0];
		aElement.textContent = location;

		// spanContent에 a 태그 추가
		spanContent.appendChild(aElement);

		// span1Element, span3Element 등을 liElement에 추가
		liElement.appendChild(span1Element);
		liElement.appendChild(span3Element);

		// li 태그를 ulElement에 추가
		ulElement.appendChild(liElement);
	}

	// ul 태그를 courseListDiv에 추가
...
function handleATagClick(index) {
	// 모든 폴리라인을 숨김
	polylines.forEach(function(line, currentIndex) {
	...
	polylines[index].polyline.setMap(map);
}
